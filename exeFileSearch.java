...
public class exeFileSearch {

    public static void main() {

        File directory = new File(System.getProperty("user.home"));

        JFrame output = new JFrame("ELXR exe SEARCH");
        ...
        JLabel info = new JLabel("EXE FILE REQUEST");
        ...
        search(directory, exeFiles);
        StringBuilder sb = new StringBuilder();



        for (File exeFile : exeFiles) {
            sb.append(exeFile.getAbsolutePath()).append("\n");
        ...
        JOptionPane.showMessageDialog(null, "Found " + exeFiles.size() + " .exe files:", "ELXR", JOptionPane.INFORMATION_MESSAGE);

    }

    public static void search(File directory, List<File> exeFiles) {
        File[] files = directory.listFiles();
        if (files != null) {
            for (File file : files) {
                if (file.isDirectory()) {
                    search(file, exeFiles);
